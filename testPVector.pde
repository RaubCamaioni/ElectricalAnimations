...
void mouseClicked() {
  ...
  if(tool.addWireIn()) {
    wires.add(new Wire(width/2, height/2, 1000, true, 1, 0));
  }


  if(tool.addWireOut()) {
    wires.add(new Wire(width/2, height/2, 1000, false, 1, 0));
  }
}
...
void twoPoleMotor() {
  float freq = .7;
  wires.add(new Wire(width/2     , height/2 + 100, 1000,  true, freq,    0));     //a
  wires.add(new Wire(width/2     , height/2 - 100, 1000, false, freq,    0));     //a'
  wires.add(new Wire(width/2 + 87, height/2 -  50, 1000,  true, freq, -120));     //b
  wires.add(new Wire(width/2 - 87, height/2 +  50, 1000, false, freq, -120));     //b'
  wires.add(new Wire(width/2 - 87, height/2 -  50, 1000,  true, freq, -240));     //c
  wires.add(new Wire(width/2 + 87, height/2 +  50, 1000, false, freq, -240));     //c'
}
class Wire implements Draggable {
  ...
  void cos(double t, double dt, float mag) {
    this.mag = (float)(mag*Math.cos(TWO_PI*freq*(t + dt) + PI/180*this.phase));
  }
